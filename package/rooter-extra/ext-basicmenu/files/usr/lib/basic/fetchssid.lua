...
function get_keys()
	local hfile = io.open("/etc/hotspot", "r")
	local cfile = io.open("/etc/ssidkeys", "w")
	cfile:close()
	cfile = io.open("/etc/ssidkeys", "a")
	if hfile ~= nil then
		repeat
			...
			if line == nil then
				...
				s, e = line:find("|")
				ssidh = line:sub(0, s-1)
				cs, ce = line:find("|", e+1)
				keysh = line:sub(cs+1)
				cfile:write(ssidh .. "|" .. keysh)
				cfile:write("\n")
			end
		until 1==0
		hfile:close()
...
str6a = ' : &nbsp;&nbsp;&nbsp;</strong></label><input type="password" style="font-size:18px;" id="'
str6b = '</br><br></td><td class="cbi-value-field" style="vertical-align:middle; text-align:right; padding:3px"><input type="button" id="'
str7 = '" value="'
str7b = '" /><input type="button" size="2" value=".." id="'
str7d = '" onclick="return showpass(this.id)"/><input type="button" id="'
...
if numnet > 0 then 
	for m=1, numnet do 
		...
		if value == nil then
			value = str7 .. str7b .. "spass" .. scan[m]["essid"] ..str7d
		else
			value = str7 .. value .. str7b .. "spass" .. scan[m]["essid"] .. str7d
		end
		...
		tab = tab .. str5 .. lblencr .. str5a .. scan[m]["encrypt"]
		if scan[m]["encryption"] == "none" then
			tab = tab .. str6b .. data .. str8 .. lblconn .. str8a
		else
			tab = tab .. str6 .. lblkey .. str6a .. "pass" .. scan[m]["essid"] .. value .. "add|" .. data .. str7a .. lblpass .. str7c .. data .. str8 .. lblconn .. str8a
		end
		wtab = wtab ..tab
	end
